...
export function Fiscalizacao() {
  ...
  const handleSubmitSatisfacao = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Nota abaixo de 4 dispara alerta por e-mail para a GSU — não bloqueia
      // nem falha o registro da avaliação se o envio der erro. É aguardado
      // (await) para não correr o risco de a requisição ser abortada caso a
      // escola feche o app/aba logo após enviar (comum em celular).
      if (satisfacaoForm.nota < 4) {
        try {
          await supabase.functions.invoke('send-outage-email', {
            body: {
          ...
          console.error('Erro ao enviar e-mail de alerta de avaliação baixa:', err);
        }
      }
